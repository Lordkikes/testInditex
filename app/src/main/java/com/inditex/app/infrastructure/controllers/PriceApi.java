...
public interface PriceApi {
    ...
    @GetMapping("/price")
    ResponseEntity<PriceDto> getProductPrice(@Parameter( name = "brandId", description = "Identificador de la cadena de la marca",
                                                      example = "1",required = true) Long brandId,
                                             @Parameter(name="date",description = "Fecha y hora de consulta del precio",
                                                     example = "2020-06-14T10:00:00",required = true) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime date,
                                             @Parameter(name="productId",description = "Identificador del producto",
                                                     example = "35455",required = true) Long productId);
}
